...
%  A description of each parameter follows:
%
%    o time_value: time value.
%
%    o time_zone: time zone.
%
*/
WizardExport int gettimeofday (struct timeval *time_value,
      ...
      if (module == (LPFNDLLFUNC2) NULL)
        {
          ...
          return((ssize_t) status.dwAvailPhys/system_info.dwPageSize/4);
        }
      status.dwLength=sizeof(status);
      if (module(&status) == 0)
        return(0L);
      return((ssize_t) status.ullAvailPhys/system_info.dwPageSize/4);
    }
    case _SC_OPEN_MAX:
      return(2048);
    default:
