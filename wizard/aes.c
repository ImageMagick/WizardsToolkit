...
WizardExport AESInfo *AcquireAESInfo(void)
{
  ...
  aes_info->encipher_key=(unsigned int *) AcquireQuantumMemory(60UL,
    sizeof(*aes_info->encipher_key));
  aes_info->decipher_key=(unsigned int *) AcquireQuantumMemory(60UL,
    sizeof(*aes_info->decipher_key));
  if ((aes_info->key == (StringInfo *) NULL) ||
      (aes_info->encipher_key == (unsigned int *) NULL) ||
      (aes_info->decipher_key == (unsigned int *) NULL))
    ThrowWizardFatalError(CipherDomain,MemoryError);
  aes_info->timestamp=time((time_t *) NULL);
  aes_info->signature=WizardSignature;
  return(aes_info);
}

/*
...
%                                                                             %
%                                                                             %
%   D e c i p h e r A E S B l o c k                                           %
%                                                                             %
%                                                                             %
%                                                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  DecipherAESBlock() deciphers a single block of ciphertext to produce a block
%  of plaintext.
%
%  The format of the DecipherAESBlock method is:
%
  ...
  const unsigned char *ciphertext,unsigned char *plaintext)
{
  ...
  */
  ...
  InitializeRoundKey(ciphertext,aes_info->decipher_key+4*aes_info->rounds,text);
  for (i=aes_info->rounds-1; i > 0;  i--)
  {
    /*
      This linear mixing step undiffuses the bits over multiple rounds.
    */
    for (j=0; j < 4; j++)
      key[j]=I[text[j] & 0xff] ^
        RotateLeft(I[(text[map[1][j]] >> 8) & 0xff] ^
        RotateLeft(I[(text[map[2][j]] >> 16) & 0xff] ^
        RotateLeft(I[(text[map[3][j]] >> 24) & 0xff])));
    AddRoundKey(key,aes_info->decipher_key+4*i,text);
  }
  for (i=0; i < 4; i++)
  {
    alpha=(text[i] & 0x000000ff) | ((text[map[1][i]]) & 0x0000ff00) |
      ((text[map[2][i]]) & 0x00ff0000) | ((text[map[3][i]]) & 0xff000000);
    key[i]=ByteSubTransform(alpha,InverseSBox);
  }
  FinalizeRoundKey(key,aes_info->decipher_key,plaintext);
  /*
    Reset registers.
  */
  alpha=0;
  (void) ResetWizardMemory(key,0,sizeof(key));
  (void) ResetWizardMemory(text,0,sizeof(text));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
WizardExport AESInfo *DestroyAESInfo(AESInfo *aes_info)
{
  ...
  WizardAssert(CipherDomain,aes_info != (AESInfo *) NULL);
  WizardAssert(CipherDomain,aes_info->signature == WizardSignature);
  if (aes_info->decipher_key != (unsigned int *) NULL)
    aes_info->decipher_key=(unsigned int *)
      RelinquishWizardMemory(aes_info->decipher_key);
  if (aes_info->encipher_key != (unsigned int *) NULL)
    aes_info->encipher_key=(unsigned int *)
      RelinquishWizardMemory(aes_info->encipher_key);
  if (aes_info->key != (StringInfo *) NULL)
    aes_info->key=DestroyStringInfo(aes_info->key);
  aes_info->signature=(~WizardSignature);
  aes_info=(AESInfo *) RelinquishWizardMemory(aes_info);
  return(aes_info);
}

/*
...
%                                                                             %
%                                                                             %
%   G e t A E S B l o c k s i z e                                             %
%                                                                             %
%                                                                             %
%                                                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  GetAESBlocksize() returns the AES blocksize.
%
%  The format of the GetAESBlocksize method is:
%
%      unsigned int *GetAESBlocksize(const AESInfo *aes_info)
%
%  A description of each parameter follows:
%
%    o aes_info: The aes info.
%
*/
...
WizardExport void SetAESKey(AESInfo *aes_info,const StringInfo *key)
{
  ...
  */
  ...
  /*
    Generate deciphering key (in reverse order).
  */
  for (i=0; i < 4; i++)
  {
