  ...
  const char *tag,const size_t offset)
{
  ...
  child->signature=WizardSignature;
  ...
  const char *path,const size_t offset)
{
  ...
  {
    GetPathComponent(components[i],SubnodePath,subnode);
  ...
  for ( ; i < (ssize_t) number_components; i++)
    components[i]=DestroyString(components[i]);
  components=(char **) RelinquishWizardMemory(components);
  return(node);
}

/*
  ...
  if (*p != '\0')
    {
      ...
      */
      ...
      utf8=(unsigned char *) RelinquishWizardMemory(utf8);
      if (base64 == (char *) NULL)
        return((char *) NULL);
...
WizardExport XMLTreeInfo *DestroyXMLTree(XMLTreeInfo *xml_info)
{
  ...
  xml_info=(XMLTreeInfo *) RelinquishWizardMemory(xml_info);
  return((XMLTreeInfo *) NULL);
}

/*
  ...
  if ((file == fileno(stdin)) || (offset < 0) ||
      (offset != (WizardOffsetType) ((ssize_t) offset)))
    {
      ...
      */
      ...
      length=(size_t) WizardMin(i+count,extent);
      xml[length]='\0';
      return(xml);
    }
  ...
  SplayTreeInfo *attributes)
{
  ...
  WizardAssert(ResourceDomain,(xml_info->signature == WizardSignature) ||
         (((XMLTreeRoot *) xml_info)->signature == WizardSignature));
  (void) LogWizardEvent(TraceEvent,GetWizardModule(),"...");
  WizardAssert(ResourceDomain,attributes != (SplayTreeInfo *) NULL);
  if (xml_info->attributes == (char **) NULL)
    return(WizardTrue);
  i=0;
  while (xml_info->attributes[i] != (char *) NULL)
        ...
        (*p == ':') || (*p < '\0'))
      {
        ...
        */
        if (root->node == (XMLTreeInfo *) NULL)
          {
            (void) ThrowWizardException(exception,GetWizardModule(),
              OptionWarning,"root tag missing");
            utf8=DestroyString(utf8);
            return(&root->root);
          }
            ...
            if (strncmp(p,"!DOCTYPE",8) == 0)
              {
                ...
                */
                ...
                if ((*p == '\0') && (terminal != '>'))
                  {
                    (void) ThrowWizardException(exception,GetWizardModule(),
                      OptionWarning,"unclosed <!DOCTYPE");
                    utf8=DestroyString(utf8);
                    return(&root->root);
                  }
                if (l != 0)
...
static char *EncodePredefinedEntities(const char *source,ssize_t offset,
  char **destination,size_t *length,size_t *extent,WizardBooleanType pedantic)
{
  char
    *canonical_content;
